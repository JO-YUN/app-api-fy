	...
	/* ---- */
	//新闻置顶
	function newTop(picUrl,id, title, date,intro) {
		var html =
			
			/*'<a href="query_news.html?id='+id+'">' +
			'	<div>' +
			'		<div class="col-xs-12 ">' +
			'			<div class="xwlb-zw-sanlie">' + title + '</div>' +
			'		</div>' +
			...
			/*'<div">' +
			'	<div class="col-xs-12  liebiao-datu "style="padding-left: 10px; padding-right: 10px;">' +
	...
	//中间两张图片
	function newTopTow(picUrl1, picUrl2, title,id) {
		var html =
		/*	'	<div>' +
			'		<div class="col-xs-12 ">' +
	...
	//下边列表
	/*function newTopThree(picUrl, title, date,id,intro,yearStr,monthStr,daysStr) {*/
	/*function newTopThree(title,id,intro,yearStr,monthStr,daysStr) {
		var html =
			'<li>' +
			'<div>' +
			'	<div class="col-xs-5">' +
			...
			'		</div>' +
			'		<a href="query_news.html?id='+id+'"><p style="font-size: 14px; font-weight: bold;">' + title + '</p></a>' +
			'	</div>' +
			'	<div class="xwlb-zw line1 " style="padding-top: 10px;">' +
			'		<a href="query_news.html?id='+id+'"><p style="font-size: 14px; ">' + intro + '</p></a>' +
			'	</div>' +
			'	<div class="xwlb-zw ">' +
			'		<a href="query_news.html?id='+id+'"><p style="font-size: 14px; font-weight: bold;float: right;padding-top: 10px;">查看更多>></p></a>' +
			'	</div>' +
			'</li>';
			return html;
			*/
	...
	function newTopThree(picUrl, title, releaseDate,id,intro) {
  	...
  	function getNewsType(){
		...
		url: "../newsType/queryNewsTypeList", //请求地址，
		data: {
		}, //data是传给后台的字段，支取5条数据
  ...
  //标题截取
   ...
   function introSubstr (str){
	...
	//AJAX方法
	function getNewsList(pageNo,pageSize,homePages,typeId){
			...
			success: function(result) {
				...
				if(topTowList.length > 0){
					for(var i = 0; i < topTowList.length; i++) {
						...
						var releaseDate = topTowList[i].releaseDate;
						...
						/*$("#newTopThree").append(newTopThree(title,id,intro,yearStr,monthStr,daysStr));*/
	...
	$(function(){
		getNewsType();
		getNewsList('1','5',"1",$("#newsTypeHidden").val())
		// alert("自动加载");
	});
	/* ---- */
	/* --轮播-- */
