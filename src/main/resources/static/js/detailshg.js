...
/*	$('.yzm').on('tap', function() {
	...
	$("#carid input").blur(function() {
		caridfunction();
	});
	
	$('#img-code').attr('src', code);

	$('#img-code').on('tap', function() {
		$('#img-code').attr('src', code + '?' + guid());
	});
...
function caridfunction(){
	var caridinput = $('#carid input').val();
	if(caridinput.length < 5 || caridinput.length > 9){
		mui.toast("车牌号格式不正确，请重新输入！")
		return false;
	}
}
