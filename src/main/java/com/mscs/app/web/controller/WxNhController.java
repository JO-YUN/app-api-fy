...
public class WxNhController {
   ...
   /* private static final Logger logger = Logger.getLogger(WxController.class);*/
    ...
    @RequestMapping(value = "/wxLoginnh", method = RequestMethod.GET)
    public String wxLogin(HttpServletRequest request, HttpServletResponse response,HttpSession httpSession) throws ParseException, IOException {
        ...
        String backUrl= Constant.server_url + "/callBacknh?n="+n;
        // 第一步：用户同意授权，获取code
    ...
    * @Date: 2018/9/25
    */
    @RequestMapping(value = "/callBacknh", method = RequestMethod.GET)
    @ResponseBody
    public void callBack(ModelMap modelMap, HttpServletRequest req, HttpServletResponse resp,HttpSession httpSession) throws ServletException, IOException {
       ...
       //接来的逻辑就是你系统逻辑了，请自由发挥
       String urlrsult = "new/wodenh.html?data="+result;
		/*
    ...
    @RequestMapping(value = "/wxLoginnhpay", method = RequestMethod.GET)
    public String wxLoginpay(HttpServletRequest request, HttpServletResponse response,HttpSession httpSession) throws ParseException, IOException {
        //这个url的域名必须要进行再公众号中进行注册验证，这个地址是成功后的回调地址
    	String n = Math.random()+"";
        String backUrl= Constant.server_url + "/callBacknhpay?n="+n;
        // 第一步：用户同意授权，获取code
        String url ="https://open.weixin.qq.com/connect/oauth2/authorize?appid="+ WXAuthUtil.APPID_NH
                + "&redirect_uri="+ URLEncoder.encode(backUrl)
                + "&response_type=code"
    ...
    * @Date: 2018/9/25
    */
    @RequestMapping(value = "/callBacknhpay", method = RequestMethod.GET)
    @ResponseBody
    public void callBackpay(ModelMap modelMap, HttpServletRequest req, HttpServletResponse resp,HttpSession httpSession) throws ServletException, IOException {
       ...
       String urlrsult = "";
		 ...
		 * urlrsult = "details.html"; } }else { urlrsult = "details.html"; }
		 */
       ReqRealNameDto dto = new ReqRealNameDto();
       dto.setOpenid(openid);
       int i = realNameMapper.checkOpenidNh(dto);
       if(i > 0) {
    	   urlrsult = "bmjf/pay";
       }else {
    	   urlrsult ="new/wodenh.html?data="+result;
       }
       resp.sendRedirect(urlrsult);
